...
def index():
    ...
    if request.method == "POST":
        ...
        # Check if request is JSON (from API client) or form data (from web interface)
        if request.is_json:
            # Handle JSON data from API client
            data = request.get_json()
            user_values["bedrooms"] = float(data["bedrooms"])
            user_values["bathrooms"] = float(data["bathrooms"])
            user_values["sqft_living"] = float(data["sqft_living"])
            user_values["sqft_lot"] = float(data["sqft_lot"])
            user_values["floors"] = float(data["floors"])
            user_values["waterfront"] = int(data["waterfront"])
            user_values["condition"] = int(data["condition"])
        else:
